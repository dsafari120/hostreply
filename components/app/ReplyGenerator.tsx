...
export default function ReplyGenerator({ config, hasSetup, onSwitchToSetup }: Props) {
  ...
  const generate = async (modifier: ReplyModifier = 'default') => {
    ...
    try {
      const res = await fetch('/api/reply', {
        ...
        body: JSON.stringify({ guestMessage: guestMsg, config, modifier }),
      })
      const data = await res.json()
      if (data.error) throw new Error(data.error)
      setReply(data.reply)
      setTag(data.tag || 'Other')
    } catch {
      ...
      {reply && (
        <div className="card border border-gray-100 space-y-3">
          <div className="flex items-center justify-between">
            <span className={`tag text-xs ${TAG_COLORS[tag] || TAG_COLORS['Other']}`}>{tag}</span>
          ...
          <div className="pt-2 border-t border-gray-100">
            <p className="text-xs text-gray-400 mb-2">Refine this reply</p>
            <div className="flex gap-2 flex-wrap">
              {(['shorter', 'warmer', 'formal', 'default'] as ReplyModifier[]).map(m => (
                <button key={m} onClick={() => generate(m)} disabled={loading} className="btn-ghost py-1.5 text-xs capitalize disabled:opacity-40">
                  {m === 'default' ? '↺ Regenerate' : m === 'shorter' ? 'Make shorter' : m === 'warmer' ? 'More warm' : 'More formal'}
                </button>
              ))}
            </div>
          </div>
